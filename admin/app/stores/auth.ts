...
export const useAuthStore = defineStore('auth', () => {
  ...
  function setAuth(data: AuthTokens) {
    ...
    if (import.meta.client) {
      localStorage.setItem('dealhub-admin-auth', JSON.stringify(data))
    }
  }

  function loadFromStorage() {
    if (import.meta.client) {
      const stored = localStorage.getItem('dealhub-admin-auth')
      if (stored) {
  ...
  function logout() {
    ...
    if (import.meta.client) {
      localStorage.removeItem('dealhub-admin-auth')
    }
  }
